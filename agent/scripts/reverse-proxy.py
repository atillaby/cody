    ...
    async with ClientSession(auto_decompress=False) as session:
        ...
        async with session.request(
            ...
            if random.random() < u2f_challenge_chance:
                ...
                proxy_response = web.StreamResponse(status=401, headers=headers)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="External auth provider test proxy server"
    )
    parser.add_argument(
        "target_url", help="Target Sourcegraph instance URL to proxy to"
    )
    parser.add_argument(
        "proxy_port",
        type=int,
    ...
    parser.add_argument(
        "u2f_challenge_chance",
        help="Defines chance that proxy will respond with U2F challenge; Accepts values from 0 to 1",
        type=float,
        nargs="?",
        default=0,
    )
