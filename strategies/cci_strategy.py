...
class Strategy(StrategyBase):
    def calculate_cci(
        self, high: List[float], low: List[float], close: List[float], period: int = 20
    ) -> float:
        try:
            ...
            # Mean Absolute Deviation hesaplaması düzeltildi
    ...
    async def check_signals(self, symbol: str):
        try:
            ...
            try:
                ...
                if not ohlcv or len(ohlcv) < 20:  # CCI için minimum veri
                ...
                if not isinstance(cci, (int, float)) or pd.isna(cci):
                    raise ValueError(f"Invalid CCI value for {formatted_symbol}")

                logger.info(f"{formatted_symbol} - Price: {current_price}, CCI: {cci}")
                ...
                if signal != "NEUTRAL":
                    await self.execute_signal(
                        ...
                        signal=signal,
                        price=current_price,
                        indicator_value=cci,  # rsi yerine indicator_value kullan
                    )
